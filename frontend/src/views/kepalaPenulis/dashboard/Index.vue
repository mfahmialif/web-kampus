<template>
  <div class="flex flex-col gap-6">
    <div>
      <h2 class="text-xl font-black" style="color: var(--text-heading)">Dashboard Kepala Penulis</h2>
      <p class="text-sm" style="color: var(--text-muted)">Pantau artikel dan akun penulis berita.</p>
    </div>
    ...
    <section class="rounded-2xl p-5" style="background: var(--bg-card); border: 1px solid var(--border)">
      <div class="flex items-center justify-between gap-3">
        <h3 class="text-lg font-black" style="color: var(--text-heading)">Artikel Penulis Terbaru</h3>
        <router-link to="/kepala-penulis/news" class="rounded-lg px-3 py-2 text-sm font-bold text-accent hover:bg-white/5">Lihat Semua</router-link>
      </div>
      <div class="mt-4 grid gap-3">
        <router-link v-for="item in latest" :key="item.id" :to="`/kepala-penulis/news/${item.id}/edit`" class="rounded-xl p-4 transition hover:bg-white/5" style="background: var(--bg-input); border: 1px solid var(--border)">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p class="font-black" style="color: var(--text-heading)">{{ item.title }}</p>
              <p class="text-sm" style="color: var(--text-muted)">{{ item.author?.name || '-' }} · {{ categoryNames(item) || '-' }}</p>
            </div>
            <span :class="statusBadge(item.status)">{{ item.status }}</span>
          </div>
        </router-link>
        <p v-if="!latest.length" class="rounded-xl p-8 text-center text-sm" style="background: var(--bg-input); color: var(--text-muted)">Belum ada artikel penulis.</p>
      </div>
    </section>
  </div>
</template>
...
const cards = computed(() => [
  ...
  { icon: 'group', label: 'Penulis', value: data.value.stats?.writers || 0 },
])
...
onMounted(async () => { data.value = (await api.get('/kepala-penulis/dashboard')).data })
</script>
