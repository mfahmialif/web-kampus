<template>
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-4">
      <router-link to="/penulis/news" class="flex items-center gap-1 text-sm font-medium" style="color: var(--text-muted)"><span class="material-symbols-outlined text-[20px]">arrow_back</span>Kembali</router-link>
      <h2 class="text-lg font-bold" style="color: var(--text-heading)">{{ isEdit ? 'Edit Artikel' : 'Tambah Artikel' }}</h2>
    </div>
    <div class="form-card rounded-2xl p-6 flex flex-col gap-5">
      ...
      <div class="flex justify-end gap-3">
        <router-link to="/penulis/news" class="rounded-lg px-5 py-2.5 text-sm font-bold" style="border:1px solid var(--border); color: var(--text-body)">Batal</router-link>
...
const error = ref('')
const form = ref({ title: '', news_category_ids: [], status: 'Published', body: '' })
...
async function loadItem(){
  const { data } = await api.get(`/penulis/news/${route.params.id}`)
  const ids = (data.categories?.length ? data.categories : [data.category].filter(Boolean)).map(item => item.id)
  form.value = { title: data.title || '', news_category_ids: ids.length ? ids : [], status: data.status || 'Published', body: data.body || '' }
...
async function submit(){
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      await api.post(`/penulis/news/${route.params.id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    } else {
      await api.post('/penulis/news', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    }
    router.push('/penulis/news')
  } catch (e) {
    const errors = e.response?.data?.errors
    error.value = errors ? Object.values(errors).flat().join(' ') : (e.response?.data?.message || 'Gagal menyimpan artikel.')
  } finally { saving.value = false }
}
onMounted(async () => { await loadCategories(); if (isEdit.value) await loadItem() })
</script>
