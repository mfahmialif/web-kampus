  ...
  <aside :class="['sidebar flex h-screen flex-col p-4 shrink-0 transition-all duration-300', collapsed ? 'sidebar-collapsed' : '']">
    <div class="flex items-center px-3 pb-4 shrink-0" :class="collapsed ? 'justify-center' : 'justify-between'">
      <div class="overflow-hidden transition-all duration-300" :style="collapsed ? 'width:0;opacity:0' : 'width:auto;opacity:1'">
        ...
        <p class="text-sm font-medium whitespace-nowrap" style="color: var(--text-muted)">Kepala Penulis</p>
      </div>
      <span v-if="collapsed" class="material-symbols-outlined text-accent text-[28px]">supervisor_account</span>
    ...
    <div class="pt-4 shrink-0">
      <button @click="authStore.logout()" class="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-accent px-4 font-bold text-btn-text shadow-[0_0_15px_rgba(37,99,235,0.3)]"><span class="material-symbols-outlined text-[20px]">logout</span><span v-if="!collapsed">Logout</span></button>
    </div>
  </aside>
</template>
...
const navSections = [
  { label: '', items: [{ icon: 'dashboard', label: 'Dashboard', route: '/kepala-penulis/dashboard' }] },
  { label: 'Artikel', items: [
    { icon: 'article', label: 'Artikel Penulis', route: '/kepala-penulis/news' },
    { icon: 'add_circle', label: 'Tambah Artikel', route: '/kepala-penulis/news/create' },
  ] },
  { label: 'Tim', items: [{ icon: 'group', label: 'Akun Penulis', route: '/kepala-penulis/writers' }] },
  { label: 'Akun', items: [{ icon: 'person', label: 'Profile', route: '/kepala-penulis/profile' }] },
]
function isActiveRoute(itemRoute) {
  return route.path === itemRoute
    || (itemRoute === '/kepala-penulis/news' && route.path.startsWith('/kepala-penulis/news') && route.path !== '/kepala-penulis/news/create')
    || (itemRoute === '/kepala-penulis/writers' && route.path.startsWith('/kepala-penulis/writers'))
}
</script>
